...
- **Rendering**: React Markdown (with GFM) for user-generated content.

## Project Structure
- `src/api/`: API layer abstraction for backend communication (e.g., `marketsApi.js`).
- `src/assets/`: Static assets, SVGs, and images.
- `src/components/`: Reusable UI components. Complex feature layouts (e.g., comments, leaderboards) are in `src/components/layouts/activity/`.
